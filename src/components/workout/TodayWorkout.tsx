...
import { CalendarDays, ChevronDown, Clock, Info, Loader2, RefreshCw } from 'lucide-react';
...
import { generateWorkoutPlan, plannedSessionForDate } from '@/lib/workout';
import {
  AGE_GROUP_LABELS,
  SESSION_FOCUS_META,
  type Exercise,
...
export default function TodayWorkout({ profile }: { profile: UserProfile }) {
  const { total, loading: logsLoading } = useFoodLogs();
  const { plan: trainingPlan, loading: planLoading } = useRacePlan();
  ...
  useEffect(() => {
    if (!logsLoading && !planLoading) {
      const planned = plannedSessionForDate(trainingPlan, dateStr());
      setPlan(generateWorkoutPlan(profile, total, planned));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profile, logsLoading, planLoading, trainingPlan]);

  if (logsLoading || planLoading || !plan) {
    return (
  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      {/* Prescribed plan session (Feature 3 reconciliation) */}
      {planned && (
        <section className="card space-y-2 border-primary-200 bg-primary-50/40">
          <div className="flex flex-wrap items-center gap-2">
            <span className="pill bg-primary-600 text-white">
              <CalendarDays size={12} /> From your training plan
            </span>
            <span className={FOCUS_PILL[SESSION_FOCUS_META[planned.focus].tone]}>
              {SESSION_FOCUS_META[planned.focus].label}
            </span>
          </div>
          <h3 className="text-lg font-bold text-slate-900">{planned.title}</h3>
          <p className="text-sm leading-relaxed text-slate-600">{planned.description}</p>
          <div className="flex flex-wrap gap-2 pt-1">
            {planned.distanceKm ? (
              <span className="pill bg-white text-slate-600">{fmt(planned.distanceKm)} km</span>
            ) : null}
            {planned.durationMinutes ? (
              <span className="pill bg-white text-slate-600">
                <Clock size={12} /> ~{planned.durationMinutes} min
              </span>
            ) : null}
          </div>
        </section>
      ...
      <section className="space-y-3">
        {planned && plan.exercises.length > 0 && (
          <p className="text-sm font-medium text-slate-500">Optional complementary moves</p>
        )}
              ...
              {isOpen && (
                <div className="animate-slide-up space-y-3 border-t border-slate-100 px-4 py-4 text-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="pill bg-slate-100 text-slate-600">~{ex.durationMinutes} min</span>
                  </div>
                </div>
              )}
            </div>
